<template>
  <div v-if="isShow" class="app-container">
      ...
      <el-form :model="queryObj" ref="queryStudentForm" :inline="true">
        ...
        <el-row>
          ...
          <el-form-item>
            ...
            <el-button style="width: 140px" type="warning" @click="goDeletedStudentView">
              已删除学生信息
            </el-button>
          </el-form-item>
        </el-row>
    ...
    <el-row :gutter="10">
      <el-col :span="4">
        <el-button style="width: 160px" type="success" size="medium" @click="goImportStudentView">导入学生信息
        </el-button>
      </el-col>
      <el-col :span="4">
        <el-button style="width: 160px" type="danger" :disabled="multiple" size="medium"
                   @click="handleBatchRemoveStudentBtn">批量删除
        </el-button>
      </el-col>
      <el-col :span="3" :offset="12">
        <el-button type="success" size="medium" icon="el-icon-upload2" @click="exportAll">导出</el-button>
      </el-col>
    </el-row>
    ...
    <el-row>
      ...
      >
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button type="text" @click="handleDeleteStudentRecordBtn(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    async getStudentRecords(page = 1) {
      this.page.page = page
      this.loading = true
      const res = await studentAPI.queryPageStudentRecords(this.page.page, this.page.limit)
    ...
    async getConditionStudentRecords(page = 1) {
      this.page.page = page
      this.loading = true
      const res = await studentAPI.queryConditionPageStudentRecords(this.page.page, this.page.limit, this.queryObj)
      this.page.total = res.data.total
      this.studentList = res.data.list
      this.loading = false
    },
    ...
    // 跳转到已删除学生信息页面
    goDeletedStudentView() {
      this.$router.push('/edu/student/list/delete')
    },
    ...
    // 删除指定学生
    handleDeleteStudentRecordBtn(row) {
      this.$confirm('是否要删除该学生?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        studentAPI.deleteStudentByNo(row.studentNo).then(res => {
          if (res.code === 0) {
    ...
    // 批量删除学生
    handleBatchRemoveStudentBtn() {
      this.$confirm('是否要删除选中的所有学生?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        studentAPI.batchDeleteStudentRecords(this.multipleSelectionStudentKey).then(res => {
          if (res.code === 0) {
            this.$message.success('删除成功!')
            this.getConditionStudentRecords()
          } else {
